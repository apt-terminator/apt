...
def main(args):
    ...
    source_all = np.stack(combined_df["normal_pool_embedding_bert"].values)
    ...
    # ============
    # Step 2: Build Graph from BERT source embeddings
    # ============
    k = 64
    ...
    # ============
    # Step 5: Encode Target BERT Embeddings
    # ============
    model.eval()
    ...
    # ============
    target_combined_df["anomaly_score_vgae_bert"] = anomaly_score_sim
    ...
    y_scores = target_combined_df["anomaly_score_vgae_bert"].values

    auc = roc_auc_score(y_true, y_scores)
    ap = average_precision_score(y_true, y_scores)
    ndcg_builtin = ndcg_score([y_true], [y_scores])

    print(f"AUC (BERT, soft VGAE): {auc:.4f}")
