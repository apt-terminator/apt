...
# Compute classic OT projection residuals
def compute_classic_ot_scores(normal_pool, target_embeds):
    dist_matrix = cdist(target_embeds, normal_pool, metric='euclidean')
    nearest_indices = np.argmin(dist_matrix, axis=1)
    projections = normal_pool[nearest_indices]
    residuals = np.linalg.norm(target_embeds - projections, axis=1)
    return residuals

# Entropy-weighted OT score
def compute_entropy_ot_scores(normal_pool, target_embeds, eta=0.2):
    cost_matrix = cdist(target_embeds, normal_pool, metric='sqeuclidean')
    gamma = np.exp(-cost_matrix / eta)
    gamma /= gamma.sum(axis=1, keepdims=True)
    entropy_scores = entropy(gamma.T)
    projections = gamma @ normal_pool
    residuals = np.linalg.norm(target_embeds - projections, axis=1)
    return entropy_scores * residuals

# Angle-based OT score
def compute_angle_ot_scores(normal_pool, target_embeds, lam=0.1):
    cost_matrix = cdist(target_embeds, normal_pool, metric='sqeuclidean')
    gamma = np.exp(-cost_matrix / 0.05)
    gamma /= gamma.sum(axis=1, keepdims=True)
    projections = gamma @ normal_pool
    norms_target = np.linalg.norm(target_embeds, axis=1)
    norms_proj = np.linalg.norm(projections, axis=1)
    dot_products = np.sum(target_embeds * projections, axis=1)
    cos_angles = dot_products / (norms_target * norms_proj + 1e-8)
    residuals = np.linalg.norm(target_embeds - projections, axis=1)
    return residuals + lam * (1 - cos_angles)

# Local density weighted score
def compute_local_density_ot_scores(normal_pool, target_embeds, k=5):
    nbrs = NearestNeighbors(n_neighbors=k).fit(normal_pool)
    distances, _ = nbrs.kneighbors(target_embeds)
    ...
    projections = gamma @ normal_pool
    residuals = np.linalg.norm(target_embeds - projections, axis=1)
    return local_density * residuals
...
def main(args):
    ...
    #############################################################################################
    ##################### OT OPTIMAL TRANSTPORT
    ######################################################



    # Assume we already have the embeddings for the normal pool and target processes using a selected LLM, e.g., distilbert
    normal_pool_embeddings = np.array(combined_df["normal_pool_embedding_distilbert"].tolist())
    target_embeddings = np.array(target_combined_df["target_embedding_distilbert"].tolist())

    # Normalize the embeddings (important for OT stability)
    normal_pool_embeddings /= np.linalg.norm(normal_pool_embeddings, axis=1, keepdims=True)
    target_embeddings /= np.linalg.norm(target_embeddings, axis=1, keepdims=True)



    # Compute all
    classic_scores = compute_classic_ot_scores(normal_pool_embeddings, target_embeddings)
    entropy_scores = compute_entropy_ot_scores(normal_pool_embeddings, target_embeddings)
    angle_scores = compute_angle_ot_scores(normal_pool_embeddings, target_embeddings)
    density_scores = compute_local_density_ot_scores(normal_pool_embeddings, target_embeddings)


    # Compute pairwise distance matrix (cost matrix) between target and normal pool
    cost_matrix = ot.dist(target_embeddings, normal_pool_embeddings, metric="euclidean")

    # Uniform distributions over both sets
    a = np.ones((target_embeddings.shape[0],)) / target_embeddings.shape[0]
    b = np.ones((normal_pool_embeddings.shape[0],)) / normal_pool_embeddings.shape[0]

    # Compute OT transport plan (sinkhorn distances are regularized and stable)
    lambda_reg = 0.1
    transport_plan = ot.sinkhorn(a, b, cost_matrix, lambda_reg)

    # Compute anomaly score: average transport cost for each target process
    anomaly_scores_ot = (transport_plan * cost_matrix).sum(axis=1)
    ...
    target_combined_df["anomaly_score_ot_distilbert"] = anomaly_scores_ot
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_ot_distilbert"]
    ot_auc_score_distilbert = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_ot_distilbert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_ot_distilbert", ascending=False)["anomaly_score_ot_distilbert"].values.reshape(1, -1)
    ot_global_ndcg_distilbert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_distilbert, ot_ap_score, ot_global_ndcg_distilbert, ndcg_at_k)
    print("OT optimized Implementation ot_auc_score_distilbert ",ot_auc_score_distilbert)

    ###############################################classic distilbert
    target_combined_df["anomaly_score_classic_ot_distilbert"] = classic_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_classic_ot_distilbert"]
    ot_auc_anomaly_score_classic_ot_distilbert = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_classic_ot_distilbert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_classic_ot_distilbert", ascending=False)["anomaly_score_classic_ot_distilbert"].values.reshape(1, -1)
    ot_ndcg_anomaly_score_classic_ot_distilbert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_anomaly_score_classic_ot_distilbert, ot_ap_score, ot_global_ndcg_distilbert, ndcg_at_k)
    print("OT   ot_auc_anomaly_score_classic_ot_distilbert ",ot_auc_anomaly_score_classic_ot_distilbert)


    ########################entropy distilbert
    target_combined_df["anomaly_score_entropy_ot_distilbert"] = entropy_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_entropy_ot_distilbert"]
    ot_auc_score_distilbert_entropy = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_entropy_ot_distilbert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_entropy_ot_distilbert", ascending=False)["anomaly_score_entropy_ot_distilbert"].values.reshape(1, -1)
    ot_global_ndcg_entropy_distilbert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_distilbert, ot_ap_score, ot_global_ndcg_entropy_distilbert, ndcg_at_k)

    print("OT   ot_auc_score_distilbert_entropy ",ot_auc_score_distilbert_entropy)


    ###########################anlgle mini lm
    target_combined_df["anomaly_score_angle_ot_distilbert"] = angle_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_angle_ot_distilbert"]
    ot_auc_score_distilbert_angle = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_angle_ot_distilbert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_angle_ot_distilbert", ascending=False)["anomaly_score_angle_ot_distilbert"].values.reshape(1, -1)
    ot_global_ndcg_angle_distilbert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_distilbert, ot_ap_score, ot_global_ndcg_angle_distilbert, ndcg_at_k)

    print("OT   ot_auc_score_distilbert_angle ",ot_auc_score_distilbert_angle)

    ###########################local density mini lm
    target_combined_df["anomaly_score_density_ot_distilbert"] = density_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_density_ot_distilbert"]
    ot_auc_score_distilbert_density = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_density_ot_distilbert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_density_ot_distilbert", ascending=False)["anomaly_score_density_ot_distilbert"].values.reshape(1, -1)
    ot_global_ndcg_density_distilbert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_distilbert, ot_ap_score, ot_global_ndcg_angle_distilbert, ndcg_at_k)

    print("OT   ot_auc_score_distilbert_density ",ot_auc_score_distilbert_density)
