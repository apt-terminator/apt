...
def main(args):
    ...
    # ============
    # Step 1: Load roberta Embeddings
    # ============
    max_source = 1000
    outlier_ids_target = target_apt_list

    source_all = np.stack(combined_df["normal_pool_embedding_roberta"].values)
    ...
    # ============
    # Step 2: Build Graph from roberta source embeddings
    # ============
    k = 64
    ...
    # ============
    # Step 5: Encode Target roberta Embeddings
    # ============
    model.eval()
    ...
    # ============
    target_combined_df["anomaly_score_vgae_roberta"] = anomaly_score_sim
    ...
    y_scores = target_combined_df["anomaly_score_vgae_roberta"].values

    auc = roc_auc_score(y_true, y_scores)
    ap = average_precision_score(y_true, y_scores)
    ndcg_builtin = ndcg_score([y_true], [y_scores])

    print(f"AUC (roberta, soft VGAE): {auc:.4f}")
