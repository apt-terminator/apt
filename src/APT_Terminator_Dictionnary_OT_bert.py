...
def main(args):
    ...
    # Assume we already have the embeddings for the normal pool and target processes using a selected LLM, e.g., bert
    normal_pool_embeddings = np.array(combined_df["normal_pool_embedding_bert"].tolist())
    target_embeddings = np.array(target_combined_df["target_embedding_bert"].tolist())
    ...
    target_combined_df["anomaly_score_ot_bert"] = anomaly_scores_ot
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_ot_bert"]
    ot_auc_score_bert = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_ot_bert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_ot_bert", ascending=False)["anomaly_score_ot_bert"].values.reshape(1, -1)
    ot_global_ndcg_bert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_bert, ot_ap_score, ot_global_ndcg_bert, ndcg_at_k)
    print("OT optimized Implementation ot_auc_score_bert ",ot_auc_score_bert)

    ###############################################classic bert
    target_combined_df["anomaly_score_classic_ot_bert"] = classic_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_classic_ot_bert"]
    ot_auc_anomaly_score_classic_ot_bert = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_classic_ot_bert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_classic_ot_bert", ascending=False)["anomaly_score_classic_ot_bert"].values.reshape(1, -1)
    ot_ndcg_anomaly_score_classic_ot_bert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_anomaly_score_classic_ot_bert, ot_ap_score, ot_global_ndcg_bert, ndcg_at_k)
    print("OT   ot_auc_anomaly_score_classic_ot_bert ",ot_auc_anomaly_score_classic_ot_bert)


    ########################entropy bert
    target_combined_df["anomaly_score_entropy_ot_bert"] = entropy_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_entropy_ot_bert"]
    ot_auc_score_bert_entropy = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_entropy_ot_bert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_entropy_ot_bert", ascending=False)["anomaly_score_entropy_ot_bert"].values.reshape(1, -1)
    ot_global_ndcg_entropy_bert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_bert, ot_ap_score, ot_global_ndcg_entropy_bert, ndcg_at_k)

    print("OT   ot_auc_score_bert_entropy ",ot_auc_score_bert_entropy)


    ###########################anlgle mini lm
    target_combined_df["anomaly_score_angle_ot_bert"] = angle_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_angle_ot_bert"]
    ot_auc_score_bert_angle = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_angle_ot_bert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_angle_ot_bert", ascending=False)["anomaly_score_angle_ot_bert"].values.reshape(1, -1)
    ot_global_ndcg_angle_bert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_bert, ot_ap_score, ot_global_ndcg_angle_bert, ndcg_at_k)

    print("OT   ot_auc_score_bert_angle ",ot_auc_score_bert_angle)

    ###########################local density mini lm
    target_combined_df["anomaly_score_density_ot_bert"] = density_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_density_ot_bert"]
    ot_auc_score_bert_density = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_density_ot_bert", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_density_ot_bert", ascending=False)["anomaly_score_density_ot_bert"].values.reshape(1, -1)
    ot_global_ndcg_density_bert = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_bert, ot_ap_score, ot_global_ndcg_angle_bert, ndcg_at_k)

    print("OT   ot_auc_score_bert_density ",ot_auc_score_bert_density)
