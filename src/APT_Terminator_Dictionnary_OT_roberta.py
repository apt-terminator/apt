...
def main(args):
    ...
    # Assume we already have the embeddings for the normal pool and target processes using a selected LLM, e.g., roberta
    normal_pool_embeddings = np.array(combined_df["normal_pool_embedding_roberta"].tolist())
    target_embeddings = np.array(target_combined_df["target_embedding_roberta"].tolist())
    ...
    target_combined_df["anomaly_score_ot_roberta"] = anomaly_scores_ot
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_ot_roberta"]
    ot_auc_score_roberta = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_ot_roberta", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_ot_roberta", ascending=False)["anomaly_score_ot_roberta"].values.reshape(1, -1)
    ot_global_ndcg_roberta = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_roberta, ot_ap_score, ot_global_ndcg_roberta, ndcg_at_k)
    print("OT optimized Implementation ot_auc_score_roberta ",ot_auc_score_roberta)

    ###############################################classic roberta
    target_combined_df["anomaly_score_classic_ot_roberta"] = classic_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_classic_ot_roberta"]
    ot_auc_anomaly_score_classic_ot_roberta = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_classic_ot_roberta", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_classic_ot_roberta", ascending=False)["anomaly_score_classic_ot_roberta"].values.reshape(1, -1)
    ot_ndcg_anomaly_score_classic_ot_roberta = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_anomaly_score_classic_ot_roberta, ot_ap_score, ot_global_ndcg_roberta, ndcg_at_k)
    print("OT   ot_auc_anomaly_score_classic_ot_roberta ",ot_auc_anomaly_score_classic_ot_roberta)


    ########################entropy roberta
    target_combined_df["anomaly_score_entropy_ot_roberta"] = entropy_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_entropy_ot_roberta"]
    ot_auc_score_roberta_entropy = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_entropy_ot_roberta", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_entropy_ot_roberta", ascending=False)["anomaly_score_entropy_ot_roberta"].values.reshape(1, -1)
    ot_global_ndcg_entropy_roberta = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_roberta, ot_ap_score, ot_global_ndcg_entropy_roberta, ndcg_at_k)

    print("OT   ot_auc_score_roberta_entropy ",ot_auc_score_roberta_entropy)


    ###########################anlgle mini lm
    target_combined_df["anomaly_score_angle_ot_roberta"] = angle_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_angle_ot_roberta"]
    ot_auc_score_roberta_angle = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_angle_ot_roberta", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_angle_ot_roberta", ascending=False)["anomaly_score_angle_ot_roberta"].values.reshape(1, -1)
    ot_global_ndcg_angle_roberta = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_roberta, ot_ap_score, ot_global_ndcg_angle_roberta, ndcg_at_k)

    print("OT   ot_auc_score_roberta_angle ",ot_auc_score_roberta_angle)

    ###########################local density mini lm
    target_combined_df["anomaly_score_density_ot_roberta"] = density_scores
    y_true = target_combined_df["label"]
    y_scores = target_combined_df["anomaly_score_density_ot_roberta"]
    ot_auc_score_roberta_density = roc_auc_score(y_true, y_scores)
    ot_ap_score = average_precision_score(y_true, y_scores)

    # Global NDCG
    relevance = target_combined_df.sort_values("anomaly_score_density_ot_roberta", ascending=False)["label"].values.reshape(1, -1)
    scores = target_combined_df.sort_values("anomaly_score_density_ot_roberta", ascending=False)["anomaly_score_density_ot_roberta"].values.reshape(1, -1)
    ot_global_ndcg_density_roberta = ndcg_score(relevance, scores)
    # NDCG@k for various cutoffs
    ndcg_at_k = {k: ndcg_score(relevance, scores, k=k) for k in [1, 5, 10, 20, 50, 1000]}
    (ot_auc_score_roberta, ot_ap_score, ot_global_ndcg_angle_roberta, ndcg_at_k)

    print("OT   ot_auc_score_roberta_density ",ot_auc_score_roberta_density)
